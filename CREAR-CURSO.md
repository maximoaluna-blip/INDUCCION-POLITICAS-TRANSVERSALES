...
`course-schema.json` **ya no promete lo que esta línea no dibuja** (19-sep-2026, ADR-067): pasó de **21 tipos a 14**, que son exactamente los que su `build-course.js` sabe renderizar — los 13 comunes más `brujula-display`. Se fueron del esquema `brujula-action`, `catalog-display`, `goal-planner`, `pdf-generator`, `practices-builder`, `courses-suggestion` y `self-assessment`, que llegaron copiados de Desarrollo Institucional y aquí no tienen dibujante.
**Y si algún día se declara uno sin `case`, el build FALLA**: hasta el 19-sep-2026 caía al `default` del `switch`, que imprimía un `<p>` vacío **sin error**, y así se publicaron ocho secciones en blanco en dos cursos de Desarrollo Institucional durante cinco días (ADR-067 — ya restauradas). Lo vigilan además `python verificar-motor.py`, que compara el esquema contra el build, y `codigo.spec.js`, que prohíbe que una sección compilada salga vacía. **Antes de usar un tipo de sección, comprobar que el `build-course.js` de la línea tiene su `case`** — no que el esquema lo admita.
